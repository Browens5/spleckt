...
A groovy 70s board game lounge rendered in isometric PlayCanvas. Sign in at the front desk with a player handle, pick a table to see its game, and start or join a match. Starting a game mints a 5-character code to share with friends. Drag the lounge (or the board during a match) to nudge the camera. Click the jukebox (or the jazz badge) for a little walking-bass lounge loop and table sound effects. Table 1 hosts checkers: the organizer picks red or black, the first two players play and later joiners watch, the camera looks straight down at the board (rotated so your color is nearest), jumps are optional, and the scoreboard tracks captures through a trophy for the winner. Table 2 hosts Scum (Presidents): 3–6 players, Ace high and 2 low, lead any single or equal set, beat with a higher set of the same length, passing is always allowed, first out of cards is President and last with cards is the Scum. Hands keep going while players stay seated — after each round Scum deals and the President takes Scum's best cards. Table 3 hosts Battleship: two players secretly place the classic five-ship fleet on a 10×10 grid, then take turns firing one shot until one fleet is sunk. Table 4 hosts Connect 4 on a standing well: the camera looks at the front of the frame, click a column to drop, four in a row wins. Any table can seat computers — one for two-player games, up to five for Scum — and computer turns pause half a second so it feels like another player moved.
