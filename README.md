...
- Paste a YouTube 360 URL and use **Save to folder & process**: the browser resolves the stream (no yt-dlp/ffmpeg install), saves the file into a folder you pick, then loads it for cubemap export. 360/EAC sources use the YouTube EAC input projection in WebGL. Optional host convert remains available only when `yt-dlp` is installed self-hosted.
