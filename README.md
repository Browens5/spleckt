...
9. **Drone Services** marketing site on `drones.spleckt.com` (landing, portfolio, contact) — not linked from the main marketing site
...
## Drone Services (`drones.spleckt.com`)

Light professional marketing site for Spleckt aerial capture: marketing photo and videography, photogrammetry, 2D/3D orthomosaics, stockpile measurements, site progress documentation, and virtual tours. Host-based routing sends `drones.spleckt.com` (and `drones.localhost:3000` in local dev) to the drone UI. The main Spleckt site does not link to it; `/drones` paths return 404 on www/apex.
...
Local Drone Services: open `http://drones.localhost:3000`.
...
| `NEXT_PUBLIC_DRONES_URL` | Optional Drone Services URL — defaults to `https://drones.spleckt.com` |
...
### Drone Services host (`drones.spleckt.com`)

| Route | Purpose |
| --- | --- |
| `/` | Drone services landing |
| `/portfolio` | Mission portfolio |
| `/contact` | Book a flight |
