...
- Portfolio lives on `portfolio.spleckt.com` (`portfolio.localhost:3000` in local dev). The first `/api/portfolio` request bootstraps demo cards; `npm run db:seed:portfolio` is optional and idempotent. Sign in as editor/admin to add images and descriptions. The main site does not link to it.
